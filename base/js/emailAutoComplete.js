...
function EmailAutoComplete(options) {
	...
	this.config = {
		...
		mailArr        : ["@qq.com","@163.com","@126.com","@189.cn","@sina.com","@hotmail.com","@gmail.com","@139.com",], //邮箱数组
	...
	_renderHTML: function(cfg) {
		...
		if(curIndex > -1){
			...
			if(/@/.test(cfg.targetVal)) {
				...
				$('.auto-tip',cfg.parentNode).css("display","block");
			}else {
				curVal = cfg.targetVal;
				$('.auto-tip',cfg.parentNode).css("display","none");
			}
			if(_cache.onlyFlag) {
				...
				$('.auto-tip',cfg.parentNode).css({'position':'absolute','top':cfg.height,'width':cfg.width - 2 + 'px','left':0,
					'border':'1px solid #ccc','z-index':10000,'display':'none'});
			}
